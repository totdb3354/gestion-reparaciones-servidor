...
class RevisionDAOARevisarTest {
    ...
    private JdbcTemplate conTelefono(String estado, int abiertos) {
        ...
        when(jdbc.query(contains("SELECT ESTADO, ID_CLI FROM Telefono"), any(RowMapper.class), eq(IMEI)))
                .thenReturn(Collections.singletonList(new Object[]{estado, null}));   // singletonList admite null (List.of no)
    ...
    @Test void recibidoPasaYCreaRevision() {
        ...
        assertEquals(RevisionDAO.ResultadoARevisar.PASADO, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
    ...
    @Test void okPasaConAvisoYCreaRevisionNueva() {
        ...
        assertEquals(RevisionDAO.ResultadoARevisar.PASADO_ESTABA_OK, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verify(jdbc).update("UPDATE Telefono SET ESTADO = 'EN_REVISION' WHERE IMEI = ?", IMEI);
        verify(jdbc).update("INSERT INTO Revision (IMEI, FECHA_CREACION) VALUES (?, NOW())", IMEI);
    }
    ...
    @Test void yaEnRevisionNoTocaNada() {
        ...
        assertEquals(RevisionDAO.ResultadoARevisar.YA_ESTABA, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verificarSinCambios(jdbc);
    }

    @Test void conTrabajoAbiertoRechazado() {
        JdbcTemplate jdbc = conTelefono("RECIBIDO", 2);
        assertEquals(RevisionDAO.ResultadoARevisar.EN_REPARACION, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verificarSinCambios(jdbc);
    }

    @Test void bloqueadoRechazado() {
        JdbcTemplate jdbc = conTelefono("BLOQUEADO", 0);
        assertEquals(RevisionDAO.ResultadoARevisar.BLOQUEADO, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verificarSinCambios(jdbc);
    }

    @Test void enviadoYDesguaceFuera() {
        assertEquals(RevisionDAO.ResultadoARevisar.FUERA, new RevisionDAO(conTelefono("ENVIADO", 0), mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        assertEquals(RevisionDAO.ResultadoARevisar.FUERA, new RevisionDAO(conTelefono("DESGUACE", 0), mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
    }

    @Test void historicoSinEstadoRechazado() {
        JdbcTemplate jdbc = conTelefono(null, 0);
        assertEquals(RevisionDAO.ResultadoARevisar.HISTORICO, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verificarSinCambios(jdbc);
    }

    @Test void imeiDesconocidoNoExiste() {
        JdbcTemplate jdbc = sinTelefono();
        assertEquals(RevisionDAO.ResultadoARevisar.NO_EXISTE, new RevisionDAO(jdbc, mock(MovimientoDAO.class)).pasarARevisar(IMEI, 3));
        verificarSinCambios(jdbc);
    }
